...
def export_opensim(selected_only=False, rename_mesh=True, grouped=False, individual=False, by_collections = False, operator=None):
    ...
    if rename_mesh:
        ...
        for obj in selected_objects:
            ## not a cloned/linked
            #if obj.data.users <= 1:
            if (obj.data not in renamed_data): ## Already named, good for cloned/linked objects, we take first object as original one
    ...
    def export_objects(export_file):
        ...
        bpy.ops.wm.collada_export(
            ...
            keep_bind_info=True,
            include_children=False, ##Rig //* False because it include everything like not work if selected_only is True
    ...
    if not grouped:
        ...
        if by_collections: ## by Collection
            ...
            bpy.ops.object.select_all(action='DESELECT')
            for obj in selected_objects:
                if not obj.name.endswith(name_suffex) and (convex_name not in obj.data.attributes):
                    obj.select_set(True)
            if len(bpy.context.selected_objects)>0:
